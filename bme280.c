...
int bme280Temperature(int *T){
	...
	ii = bme280Init(1, 0x76);
	if (ii != 0)
	{
		return -1; // problem - quit
	}

	ucTemp[0] = 0xfa; // start of data registers we want
	rc = write(file_i2c, ucTemp, 1); // write address of register to read
	i = read(file_i2c, ucTemp, 8);
	if (rc < 0 || i != 8)
	{
	...
	t = (ucTemp[0] << 12) + (ucTemp[1] << 4) + (ucTemp[2] >> 4);
	// Calculate calibrated temperature value
	// the value is 100x C (e.g. 2601 = 26.01C)
	var1 = ((((t >> 3) - (calT1 <<1))) * (calT2)) >> 11;
	var2 = (((((t >> 4) - (calT1)) * ((t>>4) - (calT1))) >> 12) * (calT3)) >> 14;
...
int bme280Humidity(int *H){
	...
	ucTemp[0] = 0xfd; // start of data registers we want
	rc = write(file_i2c, ucTemp, 1); // write address of register to read
	i = read(file_i2c, ucTemp, 8);
	if (rc < 0 || i != 8)
	{
